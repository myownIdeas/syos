...
@section('content')
    ...
    <div class="col-sm-12">
        <h4 class="section-subtitle"><b>Update Testimonial</b></h4>
        <div class="panel">
            <div class="panel-content">
                <div class="row">
                    <div class="col-md-12">
                        <form class="form-horizontal form-stripe" method="post" action="{{url('update-testimonial')}}" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="id" value="{{$testmonial->id}}">
                            <input type="hidden" name="image_name" value="{{unserialize($testmonial->image)[0]}}">
                            <div class="form-group">
                                <label for="disabled" class="col-sm-2 control-label">Name</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" value="{{$testmonial->name}}" name="name" required placeholder="Name" >
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="disabled" class="col-sm-2 control-label">Title</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="title" value="{{$testmonial->title}}" required placeholder="Title" >
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="disabled" class="col-sm-2 control-label">Designation</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="designation" value="{{$testmonial->designation}}" placeholder="Designation" >
                                </div>
                            </div>
